...
(function() {
	var noop = function() {};
	var fact = [1];
	for (var factIdx = 0; factIdx < 32; factIdx++) {
		fact[factIdx + 1] = fact[factIdx] * (factIdx + 1);
	}
	...
	function setNPerm(arr, idx, n, even) {
		...
		for (var i = 0; i < n - 1; i++) {
			...
			if (v >= 32) {
				...
				arr[i] = valh >> v & 0xf;
				var mh = (1 << v) - 1;
				valh = (valh & mh) + ((valh >> 4) & ~mh);
			} else {
				arr[i] = vall >> v & 0xf;
				var ml = (1 << v) - 1;
				vall = (vall & ml) + ((vall >>> 4) & ~ml) + (valh << 28);
				valh = valh >> 4;
			}
		}
		if (even < 0 && (prt & 1) != 0) {
